...
#need mean and std for Gaussian
...
class Serialization (  object  ) :
    ...
    def save_file  ( self,filename ) :
        pickle_file =  open ( filename+self.FILE_TYPE,'wb' )
        pickle.dump (  self,f )
        pickle_file.close (  )
    ...
    def load_file (  cls,filename ) :
        try :
            ...
            p_file = open ( filename+cls.FILE_TYPE,'rb' )
        pickle_file =  pickle.load ( p_file )
        p_file.close (  )
        return pickle_file
...
class TimingExtractor ( KeypressEventReceiver ) :
    def __init__ ( self,timing_threshold = 500 ) :
        ...
        self.hold_times = [[] for i in range ( 256 ) ]
        self.flight_time_before = [[] for i in range ( 256 ) ]
        self.flight_time_after = [[] for i in range ( 256 ) ]
    ...
    def on_key ( self,key,type,time ) :
        if type =  = self.KEY_DOWN :
            ...
            if flight_time<self.timing_threshold :
                self.flight_time_before[key].append ( flight_time )
                self.flight_time_after[self.pk].append ( flight_time )
        ...
        if type =  = self.KEY_UP :
            try :
                holding_times =  time - self.press_time.pop ( key )
            except KeyError :
                return
            if holding_times<self.timing_threshold :
                self.hold_times[key].append ( holding_times )
...
class Signature ( Serialization ) :
    ...
    def create ( keystroke_capture_data,sample_number_threshold = 4,std_dev_threshold = -1 ) :
        ...
        times =  keystroke_capture_data.feed (  TimingExtractor (  )   ) .hold_times
        data = {}
